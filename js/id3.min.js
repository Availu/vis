...
(function ()
{
    ...
    Reader.prototype.open = function (file, callback)
    {
        ...
        default:
            ...
            }, function (err, resp, xhr)
            {
                ...
                self.size = parseInt(xhr.getResponseHeader(
                    'Content-Length'));
                callback();
            });
            break;
    ...
    Reader.prototype.readLocal = function (length, position, callback)
    {
        ...
        fs.read(this.fd, buffer, 0, length, position, function (err,
            bytesRead, buffer)
        {
            if (err)
            {
    ...
    DataView.prototype.getStringUtf16 = function (length, offset, bom)
    {
        ...
        for (var i = offset; i < (offset + length); i += 2)
        {
            ...
            else if (ch >= 0x10000 && ch <= 0x10FFFF)
            {
                ...
                {
                    str += String.fromCharCode(((0xFFC00 & ch) >> 10) +
                        0xD800) + String.fromCharCode((0x3FF & ch) + 0xDC00);
                }
            }
        }
        if (useBuffer)
    ...
    DataView.prototype.getUint24 = function (offset, littleEndian)
    {
        if (littleEndian)
        {
            return this.getUint8(offset) + (this.getUint8(offset + 1) << 8) +
                (this.getUint8(offset + 2) << 16);
        }
        return this.getUint8(offset + 2) + (this.getUint8(offset + 1) << 8) +
    ...
    var id3 = function (opts, cb)
    {
        ...
        else if (typeof window !== 'undefined' && window.File && opts instanceof window
            .File)
        {
            opts = {
                file: opts,
        ...
        ID3Frame.parse = function (buffer, major, minor)
        {
            ...
            else if (header.id === 'COMM')
            {
                ...
                if (encoding === 0 || encoding === 3)
                {
                    result.value = dv.getString(-1 * variableStart,
                        variableStart);
                }
                else if (encoding === 1)
                {
                    result.value = dv.getStringUtf16(-1 * variableStart,
                        variableStart, true);
                }
                else if (encoding === 2)
                {
            ...
            else if (header.id === 'APIC')
            {
                ...
                image.type = ID3Frame.imageTypes[dv.getUint8(variableStart +
                    variableLength + 1)] || 'other';
                ...
                image.description = (variableLength === 0 ? null : dv.getString(
                    variableLength, variableStart));
        ...
        ID3Frame.parseLegacy = function (buffer)
        {
            ...
            else if (header.id === 'COM')
            {
                ...
                {
                    result.value = result.value.substr(result.value.indexOf(
                        '\x00') + 1);
                }
            }
            else if (header.id === 'PIC')
            {
                ...
                image.type = ID3Frame.imageTypes[dv.getUint8(11)] ||
                    'other';
                var variableStart = 11,
                    variableLength = 0;
                for (var i = variableStart;; i++)
        ...
        ID3Tag.parse = function (handle, callback)
        {
            ...
            handle.read(128, handle.size - 128, function (err, buffer)
            {
                ...
                if (buffer.byteLength !== 128 || dv.getString(3, null,
                    true) !== 'TAG')
                {
                    processed.v1 = true;
                    return process();
                }
                tags.v1.title = dv.getString(30, 3).replace(
                    /(^\s+|\s+$)/, '') || null;
                tags.v1.artist = dv.getString(30, 33).replace(
                    /(^\s+|\s+$)/, '') || null;
                tags.v1.album = dv.getString(30, 63).replace(
                    /(^\s+|\s+$)/, '') || null;
                tags.v1.year = dv.getString(4, 93).replace(
                    /(^\s+|\s+$)/, '') || null;
                /*
                ...
                {
                    tags.v1.comment = dv.getString(28, 97).replace(
                        /(^\s+|\s+$)/, '');
                    tags.v1.version = 1.1;
                ...
                {
                    tags.v1.comment = dv.getString(30, 97).replace(
                        /(^\s+|\s+$)/, '');
                }
                /*
            ...
            handle.read(14, 0, function (err, buffer)
            {
                 ...
                 */
                if (buffer.byteLength !== 14 || dv.getString(3, null,
                    true) !== 'ID3' || dv.getUint8(3) > 4)
                {
                    processed.v2 = true;
                    return process();
                }
                ...
                handle.read(tagSize, headerSize, function (err, buffer)
                {
                    ...
                    while (position < buffer.byteLength)
                    {
                        ...
                        {
                            slice = buffer.slice(position, position +
                                6 + dv.getUint24(position + 3));
                        }
                        else
                        {
                            slice = buffer.slice(position, position +
                                10 + dv.getUint32(position + 4)
                            );
                        }
                        frame = ID3Frame.parse(slice, tags.v2.version[
                            0]);
                        if (frame)
                        {
